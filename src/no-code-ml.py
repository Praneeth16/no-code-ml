...
# Machine Learning App Page Layout
st.set_page_config(
                    page_title="No-Code Machine Learning App to Create & Optimize Models", 
                    layout="wide"
                )

st.write("""
# No-Code Machine Learning Application to Create & Optimize Models
""")
...
# Sidebar - Specific params - need to change this
...
parameter_min_samples_leaf = st.sidebar.slider("Minimum number of samples required to be at leaf node (min_samples_leaf)", 1, 10, 2, 1)

st.sidebar.subheader("General Parameters")
parameter_random_state = st.sidebar.slider("seed number (random_state)", 0, 1000, 42, 1)
parameter_criterion = st.sidebar.select_slider("Performance measure (criterion)", options=["mse", "mae"])
parameter_bootstrap = st.sidebar.select_slider("Bootstrap samples when building trees (bootstrap", options=[True, False])
parameter_oob_score = st.sidebar.select_slider("Whether to use out-of-bag samples to estimate R^2 on unseen data (oob_score)", options=[True, False])
...
def filedownload(df):
    ...
    b64 = base64.b64encode(csv.encode()).decode() # strings <-> bytes conversations
    href = f'<a href="data:file/csv;base64,{b64}" download_url="model_performance.csv">Download CSV File</a>'
    return href
...
def build_model(df):
    ...
    # Segment data into groups based on 2 hyper-parameters
